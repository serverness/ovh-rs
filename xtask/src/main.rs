...
enum Xtask {
    #[command(about = "generate CLI and SDK code from srvn.json")]
    Generate {
        #[clap(long)]
        check: bool,
        #[clap(short = 'v', long)]
...
fn main() -> Result<(), String> {
    ...
    match xtask {
        Xtask::Generate {
            ...
            sdk,
            httpmock,
        } => generate(check, verbose, sdk, httpmock),
    }
}
...
fn generate(
    ...
    // spec_path.push("ovh.json");

    // let file = File::open(spec_path).unwrap();
    //let spec = serde_json::from_reader(file).unwrap();

    let spec = serde_json::from_value(ovh_spec::generate().unwrap()).unwrap();
    ...
    if httpmock {
        ...
        std::io::stdout().flush().unwrap();
        let code = generator.httpmock(&spec, "srvn").unwrap().to_string();
